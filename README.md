...
data/
  ...
  services.ts     転職サービス情報（提携先が決まるまでは comingSoon: true の「近日公開」表示）
lib/
...
| 転職サービスの情報・アフィリエイトリンク（提携先決定後の掲載） | `data/services.ts` |
...
- 転職サービス比較ページは、提携先が決まるまで各カードに「厳選中 近日公開！」を表示し、ボタンは無効化（クリック不可）した状態です。`data/services.ts` の該当サービスで `comingSoon: false` にし、`name` / `ageRange` / `strengths` / `features` / `recommendedFor` / `url` を入力すると、そのカードだけ通常表示・リンク有効になります。
- アクセス解析・広告タグは `app/layout.tsx` 内にコメントで設置枠を用意しています。GA4等を導入する際はここに `next/script` を追加してください。
- 結果ページ上部の「ステージビジュアル」は、現在16ステージ共通のプレースホルダー画像（`public/stage-placeholder.png`）を表示しています。ステージ専用イラストが用意でき次第、`data/results.ts` の該当ステージに `imageUrl: "/xxx.png"` を追加するだけでそのステージだけ差し替わります（未設定のステージは引き続きプレースホルダーが使われます）。
- 診断結果ページの「診断サマリー」（キャリア方向性・転職準備度・現職適合度・情報収集度）は、`data/results.ts` の各ステージの `summary` オブジェクトで1〜5の値を編集できます。
